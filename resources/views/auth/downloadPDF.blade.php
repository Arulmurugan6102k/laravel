  ...
  <div>
            ...
            <tr>
              ...
              <td class="w-1/2 align-top text-right">
                <div class="text-sm text-neutral-600">
                  ...
                  <p>Number: {{$order->customer_mob_no}}</p>
                  <p>Product type : {{$order->product_type_name}}</p>
                  <p>VAT: 23456789</p>
                  <p>9552 Vandervort Spurs</p>
                  <p>Paradise, 43325</p>
                  <p>{{ $countries[$order->customer_country_id] ?? 'Unknown' }}</p>
                  @endforeach
                </div>
              </td>
            </tr>
    ...
    <thead>
        <tr>
            ...
            <td class="border-b-2 border-main pb-3 pl-2 font-bold text-main">Product image</td>
            <td class="border-b-2 border-main pb-3 pl-2 text-right font-bold text-main">Price</td>
        </tr>
    </thead>
    <tbody>
        @php $counter = 1; @endphp
        @foreach($produts_data as $data)
        <tr>
            <td class="border-b py-3 pl-3">{{ $counter }}</td>
            <td class="border-b py-3 pl-2">{{$data->product_name}}</td>
            <td class="border-b py-3 pl-2">
                @if($data->product_image_base64)
                <img src="data:image/jpeg;base64, {{ $data->product_image_base64 }}"
                    alt="{{ $data->product_name }}" style="width: 70px; height: auto;" />
                @else
                    No image available
                @endif
            </td>
            <td class="border-b py-3 pl-2 text-right">{{$data->product_cost}}</td>
        </tr>
        @php $counter++; @endphp
        @endforeach
        <tr>
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td class="bg-main p-3 text-right">
                                                @php $total = 0; @endphp
                                                @foreach($orders_results as $order)
                                                @php $total += $order->order_amount; @endphp
                                                @endforeach
                                                <div class="whitespace-nowrap font-bold text-white">{{$total}}.Rs</div>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
